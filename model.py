...
def resample_and_save(in_path, out_path, sr):
    wav = load_audio(in_path, sr)
...
def get_embedding(path):
    ...
    with torch.no_grad():
        emb = _spk_encoder.encode_batch(wav)
...
def extract_target_from_mixture(mix_path, target_path, out_dir):
    load_models()
    os.makedirs(out_dir, exist_ok=True)

    THRESHOLD = 0.45  # IMPORTANT

    mix_8k = os.path.join(out_dir, "mixture_8k.wav")
    resample_and_save(mix_path, mix_8k, SEP_SR)

    est_sources = _sepformer_sep.separate_file(mix_8k)
    ...
    for i in range(est_sources.shape[2]):
        src_path = os.path.join(out_dir, f"source_{i+1}.wav")
        torchaudio.save(src_path, est_sources[:, :, i], SEP_SR)

        src_emb = get_embedding(src_path)
        sim = cosine_sim(target_emb, src_emb)
        similarities.append((src_path, sim))
    ...
    return best_path, similarities


def enhance_and_resample(path, out_dir):
    load_models()

    enhanced_8k = _sepformer_enh.separate_file(path)[:, :, 0]
    temp_path = os.path.join(out_dir, "enhanced_8k.wav")
    torchaudio.save(temp_path, enhanced_8k.cpu(), SEP_SR)

    final_path = os.path.join(out_dir, "enhanced_target.wav")
    resample_and_save(temp_path, final_path, TARGET_SR)
...
def run_target_speaker_extraction(mixed_audio_path, target_audio_path, out_folder):
    selected_source, sims = extract_target_from_mixture(
        mixed_audio_path, target_audio_path, out_folder
    )

    if selected_source is None:
        raise ValueError("❌ No target speaker detected")

    enhanced = enhance_and_resample(selected_source, out_folder)

    return {
        "enhanced_output_path": enhanced,
